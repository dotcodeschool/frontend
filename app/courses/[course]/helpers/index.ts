...
const getStartCourseUrl = async (
  ...
  if (isInBrowserCourse) {
    console.log(
      "[getStartCourseUrl] In-browser course, redirecting to lessons",
    );
    return lessonsUrl;
  }
  ...
  try {
    ...
    if (!sessionContext || !sessionContext.user) {
      console.log(
        "[getStartCourseUrl] No session context or user, redirecting to repo setup",
      );
      return repoSetupUrl;
    }
    ...
    if (repo && repo.test_ok === true) {
      console.log(
        "[getStartCourseUrl] Repo exists and is set up, redirecting to lessons",
      );
      return lessonsUrl;
    }
    ...
    console.log(
      "[getStartCourseUrl] Repo doesn't exist or isn't set up, redirecting to repo setup",
    );
    return repoSetupUrl;
  } catch (error) {
