...
export const getMdxCourseDetails = async (slug: string) => {
  ...
  for (const sectionSlug of sectionDirs) {
    ...
    const { data: sectionData, content: sectionContent } = matter(sectionFileContents);
    ...
    for (const lessonSlug of lessonDirs) {
      ...
      const { data: lessonData, content: lessonContent } = matter(lessonFileContents);
      ...
      if (fs.existsSync(filesDir)) {
        ...
        if (fs.existsSync(path.join(filesDir, 'source'))) {
          fileType = 'source';
        } else if (fs.existsSync(path.join(filesDir, 'template')) && 
