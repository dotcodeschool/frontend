...
export function getChangedFiles(
  ...
  const isSourceOnly =
    currentFiles.source.length > 0 && currentFiles.template.length === 0;
  const rawEditorFiles = isSourceOnly
    ? currentFiles.source
    : currentFiles.template;
  ...
  if (!prev) {
    ...
    return {
      editorFiles: rawEditorFiles,
      diffFiles: [],
      solutionFiles,
      readOnly: isSourceOnly,
    };
  }

  const prevFiles = getLessonFiles(
    courseSlug,
    prev.sectionSlug,
    prev.lessonSlug,
  );
  ...
  for (const file of rawEditorFiles) {
    ...
    const changes = diffLines(original.trim(), modified.trim(), {
