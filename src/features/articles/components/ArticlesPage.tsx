...
export default function ArticlesPage({ articles }: Props) {
  ...
  const filtered = articles.filter((a) => {
    if (
      filterTag &&
      !a.tags.some((t) => t.toLowerCase() === filterTag.toLowerCase())
    )
      return false;
    if (
      filterCategory &&
    ...
    <div>
      ...
      {isFiltered && (
        <nav className="flex items-center gap-1.5 text-sm text-content-muted mb-6">
          <a
            href="/"
            className="hover:text-content-secondary transition-colors no-underline"
          >
            Home
          </a>
      ...
      {!isFiltered && featured.length > 0 && (
        <div className="mb-12">
          <h2 className="font-heading text-xl font-semibold text-content-primary mb-4">
            Trending
          </h2>
      ...
      <div className="mb-12">
        {!isFiltered && (
          <div className="flex items-baseline justify-between mb-4">
            <h2 className="font-heading text-xl font-semibold text-content-primary">
              Browse all articles
            </h2>
