...
export function SyncStatus() {
  ...
  const config = {
    ...
    local: {
      ...
      text: isAuthenticated ? 'Saved locally — will sync when online' : 'Saved on this device',
    },
    syncing: {
      icon: (
        <svg className="w-3.5 h-3.5 text-accent animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h5M20 20v-5h-5" />
          <path strokeLinecap="round" strokeWidth={2} d="M20.49 9A9 9 0 005.64 5.64L4 4m16 16l-1.64-1.64A9 9 0 003.51 15" />
        </svg>
      ),
