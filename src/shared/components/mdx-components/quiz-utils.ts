...
export function generateQuizId(question: string, options: QuizOption[]): string {
  const str = question + "|" + options.map(o => `${o.label}:${o.correct}`).join("|")
  let hash = 0
...
export function generateFIBQuizId(question: string, answers: string[]): string {
  return generateQuizId(question, answers.map(a => ({ label: a, correct: true })))
}
