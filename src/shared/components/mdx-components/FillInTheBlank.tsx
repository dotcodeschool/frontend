...
export function FillInTheBlankQuiz({ question, answers, caseSensitive = false, explanation, placeholder = 'Type your answer here' }: Props) {
  ...
  const handleSubmit = () => {
    ...
    try {
      const stored = JSON.parse(localStorage.getItem('attemptedQuizzes') ?? '{}')
      stored[quizId] = { selectedOption: userAnswer, isCorrect: result, timestamp: Date.now() }
    ...
    <div className="border border-border rounded-lg p-6 my-6">
      ...
      {!submitted ? (
        ...
        <div className={`mt-4 p-4 rounded-lg text-sm ${isCorrect ? 'bg-green-500/10 border border-green-500/30' : 'bg-red-500/10 border border-red-500/30'}`}>
          ...
          {!isCorrect && (
            <p className="text-content-muted mb-2">Acceptable answers: {answers.join(', ')}</p>
          )}
