...
export const QuizGroupContext = createContext<QuizGroupContextType | null>(null)
  ...
  return (
    <QuizGroupContext.Provider value={{ registerQuiz, updateScore }}>
      <div className="border border-border rounded-lg p-6 my-8">
        {title && <h3 className="font-heading text-xl font-semibold text-content-primary mb-1">{title}</h3>}
        {description && <p className="text-content-muted text-sm mb-4">{description}</p>}
        <div className="flex items-center gap-3 mb-4">
          <div className="flex-1 h-2 bg-elevated rounded-full overflow-hidden">
            <div className="h-full bg-accent rounded-full transition-all" style={{ width: `${progress}%` }} />
          </div>
          <span className="text-content-muted text-xs shrink-0">{completed}/{totalQuizzes} completed</span>
        </div>
        {children}
        {allDone && (
